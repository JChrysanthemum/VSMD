...
class VsmdCanFrame(object):
    ...
    def __init__(self, message, debug=False):
        ...
        self.debug_msg += self.log_formatter("Main: ", [("Raw", str(hex(message.arbitration_id))[2:].rjust(8, "0")+"#"+raw_dm),
            ("Sender", self.channel), ("Extent ID Frame", ext_msg), ("DLC", self.dlc), ("Data Frame", data_msg)
        ])
        if self.dlc % 2 != 0:
            self.debug_msg += self.log_formatter("*** Warning ***", [("DLC Count warning", "DLC count should not be ODD")])
        if debug:
    ...
    class VsmdCanExtFrame(object):
        ...
        def __init__(self, ext_msg, debug=False):
            ...
            self.debug_msg = self.log_formatter("Extend ID Frame", [
    ...
    class VsmdCanDataFrame(object):
        ...
        def __init__(self, dlc, data_msg, cw, cmd0adr, debug=False):
            ...
            def _fill_reg_inf(_cnt: int, _data, _my_t):
                ...
                for _i in range(_cnt):
                    try:
                        ...
                        war_log = self.log_formatter("Warning", [("Message", e)])
                        print(war_log)
                        break
                return _result
            ...
            self.debug_msg += self.log_formatter("Action", [("CW Means", self.cw.name)])
            if self.cw in [CWTable.W_Data_Reg, CWTable.R_Data_Reg]:
                self.debug_msg += self.log_formatter("Data Frame - Data Reg", self.regs_values)
            elif self.cw == CWTable.CMD:
                data_frame = []
                ...
                self.debug_msg += self.log_formatter("Data Frame", [("Command", data_frame)])
            else: #CWTable.R_Stat_Reg
                keys = [x[0] for x in self.regs_values]
                if "POS" in keys or "SPD" in keys:
                    self.debug_msg += self.log_formatter("Data Frame - Stat Reg", self.regs_values)
                if self.status is not None:
                    if self.ERROR_FLG:
                        self.debug_msg += self.log_formatter("STATUS", self.status)
                    else:
                        self.debug_msg += self.log_formatter("STATUS", [("0 ERROR", "0 WARNING")])
            self.debug_msg += self.log_end()
            if debug:
                print(self.debug_msg)
...
class CANFunctionList(object):
    ...
    def move(direction: str, distance: int, bus: can.interface.Bus):
        ...
        if direction == "X":
            ...
            bus.send(str2can_msg(CommonCMD.disable_motor("ALL")))
...
class CanMsgListener(Process):
    ...
    def run(self):
        ...
        while True:
            for _msg in _bus:
                self.cnt += 1
                VsmdCanFrame = VsmdCanFrame(_msg, debug=False)
                if VsmdCanFrame.ERROR_FLG == True:
                    self.debug_msg = VsmdCanFrame.debug_msg
                    print(self.debug_msg)
                    self.terminate()
...
if __name__ == "__main__":
    ...
    while True:
        for msg in bus:
            print("\n%2d\n" % cnt)
            cnt += 1
            can = VsmdCanFrame(msg, debug=True)
    # msg = str2can_msg("0004079F#002F")
